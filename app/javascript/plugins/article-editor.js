...
ArticleEditor.opts = {
    ...
    buttons: {
        ...
        control: {
            trash: { command: 'block.remove' },
            duplicate: { command: 'block.duplicate' }
        }
    },
    embed: {
...
ArticleEditor.lang['en'] = {
    ...
    "shortcuts": {
        "meta-a": "Select all text in the block",
...
ArticleEditor.add('mixin', 'block', {
    ...
    isOffsetted: function() {
        return this._getProp('offset');
    },
...
ArticleEditor.add('module', 'block', {
    ...
    set: function($el, caretPos, e) {
        var type = $el.attr('data-arx-type');
        var instance = this.app.create('block.' + type, $el);

        if (this.activeInstance && ($el.get() === this.activeInstance.$block.get())) {
            return;
        }
    ...
    _setFocus: function(instance, caretPos, e) {
        ...
        if (instance.isOffsetted()) {
            instance.$block.addClass('arx-block-focus-offset');
        }
...
ArticleEditor.add('module', 'container', {
    init: function() {
        // local
        this.containers = {
            main: ['top', 'editor', 'source', 'statusbar'],
            top: ['toolbar']
        };
    },
    start: function() {
    ...
    _buildContainers: function() {
        ...
        for (var key in this.containers) {
            for (var i = 0; i < this.containers[key].length; i++) {
                name = this.containers[key][i];
...
ArticleEditor.add('module', 'editor', {
        ...
        for (var i = 0; i < scripts.length; i++) {
            if (scripts[i].src !== '') {
                ...
                if (src.search('instagram') !== -1) {
                    $script.attr('onload', 'window.instgrm.Embeds.process()');
                }
    ...
    _buildClasses: function() {
        var classes = ['accessibility', 'blocks', 'content', 'events', 'images', 'sync'];
...
ArticleEditor.add('class', 'editor.accessibility', {
    init: function() {},
    build: function() {
        this._buildRole();
        this._buildLabel();
    ...
    // private
    _buildRole: function() {
        this.app.editor.$editor.attr({ 'aria-labelledby': 'article-editor-voice', 'role': 'presentation' });
    },
...
ArticleEditor.add('class', 'editor.events', {
    init: function() {
        ...
        this.events = {
            editor: ['click', 'dblclick', 'mouseover', 'mouseup', 'mousedown', 'touchstart', 'contextmenu', 'drop', 'dragstart', 'dragover', 'dragleave'],
            doc: ['keydown', 'keyup', 'paste', 'cut', 'copy'],
            block: ['focus']
        };
    },
    stop: function() {
    ...
    onkeydown: function(e) {
        ...
        // cmd
        this.pressedCmd = (e.ctrlKey || e.metaKey);
    ...
    ondrop: function(e) {
        if (this._isEditorTarget(e)) {
            ...
            if (this.opts.image && this.opts.image.upload && dt.files !== null && dt.files.length > 0) {
                ...
                this.app.editor.content.drop(dt.getData("text/html"), e);
            }
        }
    ...
    _isFocusEditor: function() {
        if (this.app.popup.is() || this.app.editor.isSourceMode()) {
            return false;
        }
        else {
    ...
    _startBlurEvent: function() {
        this.app.$doc.on('click.arx-blur-event-' + this.uuid + ' touchstart.arx-blur-event-' + this.uuid + ' keyup.arx-blur-event-' + this.uuid, function(e) {
            if (this.releaseSelection === false || this.app.popup.is()) {
                return;
            }

            var data = { e: e, $element: this.app.$element, uuid: this.uuid };
            var $target = (e.type === 'click' || e.type === 'touchstart') ? this.dom(e.target) : this.dom(document.activeElement);
            var isBubble = ($target.closest('.arx-popup-' + this.uuid + ', .arx-container-' + this.uuid + ', .arx-control-' + this.uuid).length !== 0);
            if (!isBubble) {
                var instance = this.app.block.get();
                var multiple = this.app.editor.blocks.isMultiple();

                var isCmd = (e.ctrlKey || e.metaKey || e.which === 91 || e.which === 17);
                if (!isCmd && (multiple || instance || this.app.editor.isSelectedAll())) {
                    this.app.editor.setSelectedAll(false);
                    this.app.editor.removeFocus();
                    this.app.block.unset();
                    this.app.editor.blocks.unset();

                    this.app.broadcast('editor.blur', data);
                    this.hasFocus = false;
                    return;
                }
            }

            if (isBubble && this.hasFocus === false) {
                this.app.broadcast('editor.focus', data);
                this.app.editor.setFocus();
                this.hasFocus = true;
            }
    ...
    _startEvents: function() {
        for (var key in this.events) {
            ...
            for (var i = 0; i < events.length; i++) {
                $target.on(events[i] + '.arx-block-events-' + this.uuid, this['on' + events[i]].bind(this));
            }
        }
    },
...
ArticleEditor.add('module', 'link', {
    popups: {
        ...
        edit: {
            ...
            header: 'Link',
            footer: {
                insert: { title: '## link.insert ##', command: 'link.insert' },
    ...
    observe: function(obj) {
        ...
        var links = this.selection.getNodes({ type: 'inline', tags: ['a'] });
        if (links.length === 0) {
    ...
    insert: function() {
        ...
        data.url = this.content.removeHtml(data.url);
...
ArticleEditor.add('module', 'toolbar', {
    ...
    init: function() {
        ...
        this.sticky = this.app.create('toolbar.sticky');
    ...
    popup: function(args) {
        ...
        var $popup = this.app.popup.$popup;
    ...
    _createBuilderButtons: function(popup, $popup) {
        for (var name in popup.buttons) {
            this.button.create(name, popup.buttons[name], $popup);
        }
    },
    _createPopupButton: function(name, button, type, $popup) {
        this.button.create(name, this._buildPopupButton(button, type), $popup);
    },
    ...
    _buildEditorButtons: function() {
        ...
        for (var key in this.editorButtons) {
            ...
            if (key === 'templates' && (!this.opts.templates.url && !this.opts.templates.json)) continue;
...
ArticleEditor.add('class', 'toolbar.sticky', {
    init: function() {
        // services
        this.utils = this.app.create('utils');
    ...
    is: function() {
        return this.app.container.$toolbar.hasClass('arx-top-sticky');
    },
    reset: function() {
        var $top = this.app.container.$top;
        var $toolbar = this.app.container.$toolbar;

        $top.css('height', '');
        $toolbar.removeClass('arx-top-sticky');
        $toolbar.css({ position: '', top: '', left: '', width: '' });

        this._updatePopup();
    },
    build: function() {
        this.$scrollTarget.on('scroll.arx-toolbar-' + this.uuid, this._doStickable.bind(this));
        this.app.$win.on('resize.arx-toolbar-' + this.uuid, this._doStickable.bind(this));

        if (this.utils.isScrollTarget()) {
            this.app.$doc.on('scroll.arx-toolbar-' + this.uuid, this._doStickable.bind(this));
        }
    ...
    // private
    _doStickable: function() {

        var $container = this.app.container.$main;
        if (this._hasHiddenTarget($container)) {
            return;
        }
        if (!this._isMustStickable($container)) {
            this.reset();
            return;
        }
        ...
        var toolbarWidth = $container.width();
        var toolbarOffset = this.opts.toolbar.stickyTopOffset;

        var box = this._getBox($container);
        var scrollTop = this.$scrollTarget.scrollTop();
        var scrollOffset = (scrollTop + toolbarOffset);

        if (this.utils.isScrollTarget()) {
            var borderTop = parseInt(this.$scrollTarget.css('border-top-width'));
            toolbarOffset = toolbarOffset + this.$scrollTarget.offset().top + borderTop - this.app.$doc.scrollTop();
            scrollOffset = scrollTop;
        }

        var top = (this.utils.isMobile()) ? (scrollOffset - box.start - 1) : 0;

        if (scrollOffset > box.start && scrollOffset < box.end) {

            var position = (this.utils.isMobile()) ? 'absolute' : 'fixed';

            $top.height(toolbarHeight);
            $toolbar.addClass('arx-top-sticky');
            $toolbar.css({
                position: position,
                top: (top + toolbarOffset) + 'px',
    ...
    _updatePopup: function() {
        if (this.app.popup.is()) {
            this.app.popup.updatePosition();
        }
    },
    _isMustStickable: function($container) {
        var editorHeight = this.app.editor.$editor.height();
        if ((editorHeight < this.opts.toolbar.stickyMinHeight)
            || this.app.editor.isSourceMode() || this.app.editor.isEmpty()) {
            return false;
        }

        return true;
    },
    _getBox: function($container) {
        var containerOffset = (this.utils.isScrollTarget()) ? $container.position() : $container.offset();
        var start = containerOffset.top;
        var end = start + $container.height() - this.opts.toolbar.stickyTolerance;
    ...
    _hasHiddenTarget: function($container) {
...
ArticleEditor.add('module', 'control', {
    ...
    _updatePosition: function(e, instance) {
        ...
        if (this.utils.isScrollTarget()) {
            ...
            offset.top = offset.top - targetOffset.top - borderTop + $target.scrollTop();
            offset.left = offset.left - targetOffset.left - borderLeft;
        }

        var expanded = instance.isOffsetted();
        var topOutlineFix = (expanded) ? -6 : -2;
        var leftOutlineFix = (expanded) ? -27 : -23;

        this.$control.css({
            top: (offset.top + topOutlineFix) + 'px',
            left: (offset.left + leftOutlineFix) + 'px'
        });
    },
    _build: function() {
    ...
    _buildButtons: function(instance) {
        for (var key in this.buttons) {
            ...
            if (instance.isAllowedButton(this.buttons[key], 'control')) {
                this.button.create(key, this.buttons[key], this.$control);
            }
        }
    }
});
...
ArticleEditor.add('module', 'popup', {
    ...
    start: function() {
        ...
        this.$container.addClass('arx-popup-container arx-popup-hide arx-popup-' + this.uuid);
    ...
    isForm: function() {
        return (this.$popup && this.$popup.hasClass('arx-popup-form'));
    },
    ...
    getData: function(name) {
        if (this.isForm() && typeof name === 'undefined') {
            return this.$popup.serialize(true);
        }
    ...
    getButton: function(name) {
        if (this.isForm() && this.$footer) {
            return this.$footer.find('[data-name=' + name + ']');
        }
    },
    getInput: function(name) {
        if (this.isForm()) {
            return this.$popup.find('[name=' + name + ']');
        }
    },
    ...
    open: function($btn) {
        if ($btn) {
            ...
            var cBtnName = this.$container.attr('data-btn-name');

            if (this.is() && btnName === cBtnName) {
                this.close();
                return;
            }
        ...
        // close all popups
        this.app.$body.find('.arx-popup-container').not('.arx-popup-' + this.uuid).addClass('arx-popup-hide');
        ...
        // in
        if ($btn) {
            $btn.addClass('arx-popup-in-' + this.uuid);
            this.$container.attr('data-btn-name', btnName);
        }
        ...
        // events
        this.app.$doc.on('keydown.arx-popup-' + this.uuid, this.handleKeyboard.bind(this));
        this.app.$doc.on('click.arx-popup-' + this.uuid, this.handleClick.bind(this));
        this.app.$win.on('resize.arx-popup-' + this.uuid, this.handleResize.bind(this));
        this.$scrollTarget.on('scroll.arx-popup-' + this.uuid, this.handleResize.bind(this));
    ...
    close: function(animate) {
        ...
        this.app.$body.find('.arx-popup-in-' + this.uuid).removeClass('arx-popup-in-' + this.uuid);
        this.app.$doc.off('.arx-popup-' + this.uuid);
        this.app.$win.off('.arx-popup-' + this.uuid);
        this.$scrollTarget.off('.arx-popup-' + this.uuid);
        ...
        if (animate !== false)  {
            ...
            this.$container.one('animationend webkitAnimationEnd', function(e) {
                ...
                this.$container.removeAttr('data-btn-name');
            }.bind(this));
        }
        else {
            this.$container.addClass('arx-popup-hide');
        ...
        this.$container.find('.arx-popup-footer-btn').off('.arx-popup-footer-btn-' + this.uuid);
        this.$container.find('.arx-button').off('.arx-button-' + this.uuid);
        this.$container.find('.arx-popup-event').off('.arx-popup-event-' + this.uuid);
    ...
    handleResize: function() {
        ...
        if (this.$popup.hasClass('arx-popup-adaptive')) {
            ...
            var value = (width - widthTolerance) + 'px';
            this.$container.css({
                'max-width': value,
                'min-width': value
            });
        ...
        var $target = this.utils.getScrollTarget();
        var offset = (this.utils.isScrollTarget()) ? this.$container.position() : this.$container.offset();
        var top = (this.app.toolbar.sticky.is()) ? this.app.toolbar.$toolbar.height() : offset.top - $target.scrollTop();
        ...
        this.$container.css('max-height', cropHeight + 'px');
    },
    handleClick: function(e) {
        var $el = this.dom(e.target);
        var isPopup = ($el.closest('.arx-popup-' + this.uuid).length !== 0);
        var isPopupIn = (this.is() && $el.closest('.arx-popup-in-' + this.uuid).length !== 0);

        if (!isPopup && !isPopupIn) {
            this.close();
        }
    },
    ...
    _buildBody: function() {
        if (this.params.form) {
            this.$popup = this.dom('<form>');
            this.$popup.addClass('arx-popup-form');
        }
        else {
            this.$popup = this.dom('<div>');
        }

        this.$popup.addClass('arx-popup');
        this.$container.append(this.$popup);
    },
    _buildHeader: function() {
        if (this.params.header) {
            this.$header = this.dom('<header>');
            this.$header.addClass('arx-popup-header');
            this.$header.html(this.lang.parse(this.params.header));
    ...
    _buildForm: function() {
        ...
        for (var key in this.params.form) {
            ...
            // append
            this.app.popup.$popup.append($item);
        }
    },
    ...
    _createFormItem: function() {
        return this.dom('<div>').addClass('form-item');
    },
    _catch: function(e) {
        e.preventDefault();
        e.stopPropagation();
...
ArticleEditor.add('module', 'template', {
    ...
    observe: function(obj, name) {
        return (this._isTemplates()) ? obj : false;
    },
    buildPopup: function(args) {
        ...
        for (var key in templates) {
            ...
            this.button.create(key, obj, $popup);
        }
    },
...
ArticleEditor.add('module', 'shortcuts', {
    ...
    buildPopup: function(args) {
        this.app.popup.build();
        this.app.popup.$popup.addClass('arx-popup-shortcuts');
    ...
    _handler: function(e, keys, obj) {
        ...
        for (var i = 0; i < len; i++) {
            if (possible[keys[i]]) {
                ...
                e.preventDefault();
                this.triggered = true;
                this.app.api(obj.command, { params: obj.params });
                return;
            }
        }
    }
...
ArticleEditor.add('module', 'table', {
    popups: {
        config: {
            ...
            header: 'Table Cell',
            footer: {
                insert: { title: '## table.save ##', command: 'table.save' },
...
ArticleEditor.add('module', 'image', {
    popups: {
        config: {
            ...
            header: 'Image',
            footer: {
                insert: { title: '## image.save ##', command: 'image.save' },
    ...
    buildConfig: function(args) {
        ...
        if (this.opts.image.upload) {
            ...
            // append to popup
            this.app.popup.$popup.prepend($formitem);
    ...
    buildPopup: function(args) {
        // build popup
        this.app.popup.build({ adaptive: true });
        this.app.popup.open(args.$btn);
        this.app.popup.$popup.addClass('arx-popup-upload-container');
        ...
        if (this.opts.image.url) {
            ...
            var $formitem = this.dom('<div>');
            $formitem.addClass('arx-form-flex-item');

            $formitem.append(this.$urlinput);
            $formitem.append(this.$urlbutton);
            this.app.popup.$popup.append($formitem);
            this.$urlinput.focus();
        }
        ...
        if (this.opts.image.url && (this.opts.image.upload || this.opts.image.select)) {
            ...
            this.app.popup.$popup.append($section);
        }
        ...
        if (this.opts.image.upload) {
            ...
            this.$uploaditem = this.dom('<div>');
            this.$uploaditem.addClass('arx-popup-upload');
            this.app.popup.$popup.append(this.$uploaditem);
        }
        ...
        if (this.opts.image.select) {
            ...
            this.app.popup.$popup.append(this.$box);
...
ArticleEditor.add('module', 'embed', {
    popups: {
        edit: {
            ...
            header: '## embed.embed ##',
            footer: {
                insert: { title: '## embed.insert ##', command: 'embed.insert' },
    ...
    start: function() {
        this.app.control.add('editembed', { iconname: 'edit', command: 'embed.edit', blocks: ['embed'] });
    },
    observeAdd: function(obj, name) {
...
ArticleEditor.add('module', 'grid', {
    ...
    buildPopup: function() {
        ...
        this.app.popup.$popup.addClass('arx-popup-grid');
        ...
        for (var i = 0; i < patterns.length; i++) {
            ...
            this.button.create(i, obj, $popup);
        }
    },
    transform: function() {
        this.app.popup.close();
...
ArticleEditor.add('module', 'code', {
    popups: {
        edit: {
            ...
            header: '## code.code ##',
            footer: {
                insert: { title: '## code.insert ##', command: 'code.insert' },
    ...
    start: function() {
        this.app.control.add('editcode', { iconname: 'edit', command: 'code.edit', blocks: ['code'] });
    },
    observeAdd: function(obj, name) {
    ...
    _insert: function(change) {
        ...
        var instance = this.app.create('block.code', $code);
...
ArticleEditor.add('module', 'snippet', {
    ...
    observeAdd: function(obj, name) {
        return (this._isSnippets()) ? obj : false;
    },
    buildPopup: function() {
        // build popup
        this.app.popup.build();
...
ArticleEditor.add('service', 'button', {
    create: function(name, obj, $container) {
        obj = this._build(obj);
        if (obj) {
            return this._create(name, obj, $container);
        }
    },
    set: function($el, name, obj) {
        obj = this._build(obj);
        if (obj) {
    ...
    _buildTooltip: function() {
        var tooltip = (this.title) ? this.title.replace(/(<([^>]+)>)/gi, '') : '';
...
ArticleEditor.add('service', 'content', {
    ...
    clean: function(html) {
        ...
        // remove tags
        var exceptedTags = this.opts.paste.blockTags.concat(this.opts.paste.inlineTags);
    ...
    parse: function(html, convertPlainText, pasteMarker) {
        ...
        // remove inline style
        $wrapper.find(this.opts.tags.inline.join(',')).removeAttr('style');
        ...
        // cache block style
        $wrapper.find(this.opts.tags.block.join(',') + ',img').each(this.cacheBlocksStyle.bind(this));
    ...
    unparse: function(html, paste) {
        ...
        // release cache styles & remove empty class
        $blocks.each(function(node) {
            var $el = this.dom(node);
    ...
    encodeCode: function(html) {
        ...
        html = this._convertBlockLevelCode(html);
        html = this._convertTmpTags(html, ['code', 'pre', 'p']);

        var utils = this.app.create('utils');
        var $wrapper = utils.createWrapper(html);

        $wrapper.find('[data-arx-tmp-tag=code], [data-arx-tmp-tag=pre]').each(this._encodeNode.bind(this));

        this._revertTmpTags($wrapper, ['code', 'pre', 'p']);

        html = utils.getWrapperHtml($wrapper);
        html = this._revertBlockLevelCode(html);
    ...
    _encodeNode: function(node) {
        ...
        if ($node.attr('data-arx-tmp-tag') === 'pre'
            && first && first.tagName === 'DIV' && $first.attr('data-arx-tmp-tag') === 'code') {
            return;
        }

        this._revertTmpTags($node, ['code', 'pre', 'p']);

        html = $node.html();
        html = this._replaceBlockLevelCode(html);
    ...
    _replaceBlockLevelCode: function(html) {
        var tags = this.opts.tags.incode;
        for (var i = 0; i < tags.length; i++) {
            html = html.replace('####/' + tags[i] + '####', '</' + tags[i] + '>');
            html = html.replace(new RegExp('####' + tags[i] + '(.*?)####', 'gi'), '<' + tags[i] + '$1>');
        }

        return html;
    },
    _convertBlockLevelCode: function(html) {
        var tags = this.opts.tags.incode;
        for (var i = 0; i < tags.length; i++) {
    ...
    _revertBlockLevelCode: function(html) {
        var tags = this.opts.tags.incode;
        for (var i = 0; i < tags.length; i++) {
            html = html.replace('####/' + tags[i] + '####', '</' + tags[i] + '>');
            html = html.replace('####' + tags[i] + '(.*?)####', '<' + tags[i] + '$1>');
        }

        return html;
    },
    _convertTmpTags: function(html, tags) {
        for (var i = 0; i < tags.length; i++) {
            html = html.replace(new RegExp('<' + tags[i] + '(.*?)>', 'gi'), '<div data-arx-tmp-tag="' + tags[i] + '"$1>');
            html = html.replace(new RegExp('</' + tags[i] + '>', 'gi'), '</div>');
        }

        return html;
    },
    _revertTmpTags: function($wrapper, tags) {
        var self = this;
        for (var i = 0; i < tags.length; i++) {
            var selector = '[data-arx-tmp-tag=' + tags[i] + ']';
            $wrapper.find(selector).replaceWith(function(node) {
                return '<' + tags[i] + self._revertAttributes(node) + '>' + node.innerHTML + '</' + tags[i] + '>';
            });
        }
    },
    _revertAttributes: function(node) {
        var attrs = node.attributes, i = attrs.length, attr;
        var except = 'data-arx-tmp-tag';
        var str = '';
        while (i--) {
            attr = attrs[i];
            if (except !== attr.name) {
    ...
    _parseWordLists: function($wrapper) {
        ...
        $wrapper.find('p').each(function(node) {
            ...
            if (currentLevel !== null) {
                ...
                $item = self.dom('<li>' + $node.html().trim() + '</li>');
                if ($list === null) {
                    $node.before(listTag);
                    $list = $node.prev();
                }
    ...
    getNodes: function(data) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            if (data) {
                // by type
                if (data.type) {
                    if (data.type === 'inline' && !utils.isInlineTag(nodes[i].tagName)) continue;
                }
                // by tag
                if (data.tags) {
                    if (nodes[i].tagName && data.tags.indexOf(nodes[i].tagName.toLowerCase()) === -1) continue;
                }
            }
...
ArticleEditor.add('service', 'utils', {
    ...
    scrollToElement: function($el, tolerance) {
        if (!this.isElementVisible($el)) {
            ...
            setTimeout(function() {
                if (this.app.toolbar.sticky.is()) {
                    value = value - this.app.toolbar.$toolbar.height();
                }
